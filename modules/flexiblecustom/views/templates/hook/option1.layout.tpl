{if $groups && $groups|@count > 0}
    <div class="flex-layout2 clearfix">
        <!-- Module header -->
        <div>
            <div class="box-title clearfix">
                <span class="navbar-brand">{$moduleName}</span>
                <div class="pull-right clearfix">
                    <a class="navPrev" data-module="{$moduleId}" href="javascript:void(0)"></a>
                    <a class="navNext" data-module="{$moduleId}" href="javascript:void(0)"></a>
                </div>
            </div>
        </div>
        <!-- END Module header -->
        <!-- Module content -->        
        <div class="clearfix module-contents">
            <div class="row">
                <!-- Module Left Content -->
                <div class="col-lg-3 col-md-4 col-sm-6 list">
                    <div class="box ">
                        <!-- List group -->
                        <ul id="module-{$moduleId}" class="ul-list">
                        {foreach from=$groups item=group name=ojb}
                            ...
                            <li data-group="{$group.id}" data-module="{$moduleId}" class="active">
                                <a href="javascript:void(0)">
                                    {if isset($group.icon) &&  $group.icon|strlen>0}
                                    <img class="cat-img" src="{$livePath}icons/{$group.icon}" alt="{$group.title}" />
                                    {/if}
                                    <span {if isset($group.icon) &&  $group.icon|strlen>0}class="mar-l-15"{/if}>{$group.title}</span>
                                </a>
                            </li>
                            {else}
                            <li data-group="{$group.id}" data-module="{$moduleId}">
                                <a href="javascript:void(0)">
                                    <img class="cat-img" src="{$livePath}icons/{$group.icon}" alt="{$group.title}" />
                                    <span class="mar-l-15">{$group.title}</span>
                                </a>
                            </li>
                            {/if}        
                        {/foreach}
                        ...
                        <div class="flexible-custom-banners">
                            {foreach from=$banners item=banner}
                                <div class="banner item">
                                    <a href="{$banner->link}"><img src="{$banner->src}" alt="{$banner->alt}" class="replace-2x img-responsive" /></a>
                                </div>                                                    
                            {/foreach}                                        
                        </div>
                        {/if}
                ...
                <!-- END Module Left Content -->
                <!-- Module Right Content -->
                <div class="col-lg-9 col-md-8 col-sm-6 sections flex-layout2-sections clearfix">
                    {foreach from=$groups item=group name=ojb7}
                    {if $smarty.foreach.ojb7.first}
                    <div style="display:block" data-module="{$moduleId}" data-group="{$group.id}" data-h-per-w="{$h_per_w}" id="section-{$moduleId}-{$group.id}" class="clearfix flex-section">
                        <div id="section-inner-{$moduleId}-{$group.id}" data-last-page="1" data-last-left="0" data-loaded="1" data-current-page="1" data-total-item="{$group.products|@count}" class="clearfix flex-section-inner" data-h-per-w="{$h_per_w}">
                            ...
                            {foreach from=$imginfo item=imgitem}
                                ...
                                {/if}
                            {/foreach}
                            <div class="item page-style3" itemscope itemtype="http://schema.org/Product">
                                ...
                                <div class="text-center">
                                    <div class="item-other-info">                    
                                        ...
                                        <div class="item-rate text-center clearfix">
                                            ...
                                            <div class="add-to-cart">
                                                {if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && $product.customizable != 2 && !$PS_CATALOG_MODE}
                                                    ...
                                                    {/if}
                                                {/if}                                                 
        										<a title="{l s='Add to Wishlist'}" class="addToWishlist" href="javascript:void(0)" data-rel="{$product.id_product}" onclick="javascript: WishlistCart('wishlist_block_list', 'add', '{$product.id_product}', false, 1); return false;"><i class="fa fa-add-to-wishlist"></i></a>
                            ...
                            {/foreach}
                            {else}
                            <div class="option1-noproduct col-xs-12">{l s='Sorry! There are no products' mod='flexiblecustom'}</div>
                            {/if}
                        </div>
                    </div>
                    {else}
                    <div style="display:none" data-module="{$moduleId}" data-group="{$group.id}" data-h-per-w="{$h_per_w}" id="section-{$moduleId}-{$group.id}" class="clearfix flex-section">
                        <div id="section-inner-{$moduleId}-{$group.id}" data-last-page="1" data-last-left="0" data-loaded="1" data-current-page="1" data-total-item="{$group.products|@count}" class="clearfix flex-section-inner" data-h-per-w="{$h_per_w}">
                            {if $group.products|@count >0}
                            {foreach from=$group.products item=product name=products}
                            {$imginfo = Image::getImages(Language::getIdByIso($lang_iso),$product.id_product)}
                                    ...
                                    <div class="item-other-info">                    
                                        ...
                                        <div class="item-rate text-center">
                                            ...
                                            <div class="add-to-cart">
                                                ...
                                                {if isset($comparator_max_item) && $comparator_max_item}
                                                    {if $product.id_product|@in_array:$compareProductIds}
                                                        <a class="add_to_compare compare-checked" title="{l s='Add to compare'}" href="{$product.link|escape:'html':'UTF-8'}" data-id-product="{$product.id_product}"><i class="fa fa-exchange"></i></a>
                                                    {else}
                                                        <a class="add_to_compare" title="{l s='Add to compare'}" href="{$product.link|escape:'html':'UTF-8'}" data-id-product="{$product.id_product}"><i class="fa fa-exchange"></i></a>
                                                    {/if}
                                                {/if}                                                    
                                            </div>
                                        </div>
