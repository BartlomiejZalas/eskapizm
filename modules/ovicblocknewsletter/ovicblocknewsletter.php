...
class Ovicblocknewsletter extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Fashion - Newsletter block');
	...
	{
		if (!parent::install() || !$this->registerHook(array('header', 'HOOK_NEWSLETTER', 'actionCustomerAccountAdd')))
			return false;
	...
	protected function sendVerificationEmail($email, $token)
	{
		$verif_url = Context::getContext()->link->getModuleLink(
			'ovicblocknewsletter', 'verification', array(
				'token' => $token,
			)
		);
	...
	public function hookDisplayLeftColumn($params)
	{
		...
		return $this->display(__FILE__, 'ovicblocknewsletter.tpl');
	}
    public function hookHOOK_NEWSLETTER($params)
	{
		$this->_prepareHook($params);
	...
	{
		$this->context->controller->addCSS($this->_path.'ovicblocknewsletter.css', 'all');
		$this->context->controller->addJS($this->_path.'ovicblocknewsletter.js');
	}
	...
	{
		//if e-mail of the created user address has already been added to the newsletter through the ovicblocknewsletter module,
		//we delete it from ovicblocknewsletter table to prevent duplicates
