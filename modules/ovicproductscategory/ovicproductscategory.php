...
class OvicProductsCategory extends Module
{
	private $html;

	public function __construct()
	{
		$this->name = 'ovicproductscategory';
		$this->version = '1.6.2';
		...
		$this->displayName = $this->l('Fashion - Products in the same category');
		$this->description = $this->l('Adds a block on the product page that displays products from the same category.');
	}

	public function install()
	{
		Configuration::updateValue('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE', 0);
		$this->_clearCache('ovicproductscategory.tpl');
	...
	{
		Configuration::deleteByName('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE');
		$this->_clearCache('ovicproductscategory.tpl');
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitCross') &&
			Tools::getValue('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE') != 0 &&
			Tools::getValue('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE') != 1
		)
			$this->html .= $this->displayError('Invalid displayPrice.');
		elseif (Tools::isSubmit('submitCross'))
		{
			Configuration::updateValue(
				'OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE',
				Tools::getValue('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE')
			);
			$this->_clearCache('ovicproductscategory.tpl');
	...
	public function hookProductFooter($params)
	{
		...
		$cache_id = 'ovicproductscategory|'.$id_product.'|'.(isset($params['category']->id_category) ? (int)$params['category']->id_category : $product->id_category_default);

		if (!$this->isCached('ovicproductscategory.tpl', $this->getCacheId($cache_id)))
		{
			...
			if (is_array($category_products) && count($category_products))
			{
				...
				if (Configuration::get('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE'))
				{
					foreach ($category_products as $key => $category_product)
					{
						if ($category_product['id_product'] != $id_product)
			...
			$this->smarty->assign(
				array(
					...
					'ProdDisplayPrice' => Configuration::get('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE')
				)
			);
		}

		return $this->display(__FILE__, 'ovicproductscategory.tpl', $this->getCacheId($cache_id));
	}
			...
			'form' => array(
				...
				'input' => array(
					array(
						...
						'name' => 'OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE',
						'values' => array(
							array(
								'id' => 'active_on',
								'value' => 1,
	...
	{
		return array(
			'OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE' => Tools::getValue(
					'OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE',
					Configuration::get('OVIC_PRODUCTSCATEGORY_DISPLAY_PRICE')
				),
		);
	}
