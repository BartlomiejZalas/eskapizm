...
class CategorySlide extends ObjectModel
{
	public $id_category;
    public $title;
	public $description;
	...
	public static $definition = array(
		'table' => 'categoryslider_slides',
		'primary' => 'id_categoryslider_slides',
		'multilang' => true,
		'fields' => array(
...
//        die();
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'categoryslider` (`id_shop`, `id_categoryslider_slides`)
...
//        die();
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'categoryslider`
			WHERE `id_categoryslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_categoryslider_slides` = hs.`id_categoryslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_categoryslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'categoryslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'categoryslider` hs ON (hss.`id_categoryslider_slides` = hs.`id_categoryslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new CategorySlide($row['id_slide']);
