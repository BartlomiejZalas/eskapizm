...
<div class="shopping_cart_container{if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
	<div class="shopping_cart">
		<a class="{if isset($current_id_option)&& ($current_id_option ==1 || $current_id_option == 3)}mainColorBgHoverOnly {/if}mainBorderColor" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			{*}<b>{l s='Cart' mod='blockcart'}</b>{*}
            {if isset($current_id_option)&& ($current_id_option ==2 || $current_id_option ==4 || $current_id_option ==5)}{l s='My Cart' mod='blockcart'}{/if}
			...
			<span class="ajax_cart_no_product{if $cart_qties > 0} unvisible{/if}">{convertPrice price=0.00}</span>
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
						{if $products}
							<dl class="products">
								{foreach from=$products item='product' name='myLoop'}
									...
									<dt data-id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}" class="mainHoverColor {if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										<a class="cart-images mainBorderHoverOnly" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default')}" alt="{$product.name|escape:'html':'UTF-8'}" /></a>
										<div class="cart-info">
											<div class="product-name">
												<a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:150:'...'|escape:'html':'UTF-8'}</a>
											</div>
                                            ...
                                            <span class="quantity-formated">{l s='Qty:'}<span class="quantity">{$product.cart_quantity}</span></span>
										</div>
						...
						<div class="cart-prices">
							...
							<div class="cart-prices-line last-line">
								<span>{l s='Cart subtotal' mod='blockcart'}:</span>
						...
						<p class="cart-buttons">
                            <a class="btn btn-default button button-small mainBorder" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span class="btnbgcolorhover">
									{l s='View Cart' mod='blockcart'}<i class="icon-angle-right right"></i>
								</span>
							</a>
							<a id="button_order_cart" class="btn btn-default button button-small mainBorder" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span class="btnbgcolorhover">
									{l s='Checkout' mod='blockcart'}<i class="icon-angle-right right"></i>
								</span>
							</a>
