...
{if $errors|@count == 0}
	...
	{/if}
	<div class="primary_block {if ($hide_left_column and $hide_right_column)} full-width{/if}{if !$hide_left_column and !$hide_right_column} both-width{/if}" itemscope itemtype="http://schema.org/Product">
		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-5">
			...
			<div id="image-block" class="clearfix">
				...
				{/if}
				{if $product->on_sale}
					<span class="sale-box no-print {if isset($product->new) && $product->new != 1}has-not-new{/if}">
				...
				{if $have_image}
					<span id="view_full_size">
						{if $jqZoomEnabled && $have_image && !$content_only}
                            ...
                            {if !$content_only}
								<span class="span_link no-print">{l s='Hover on the image to zoom'}</span>
							{/if}
						{else}
											...
											<div class="attribute_list">
												...
												{elseif ($group.group_type == 'radio')}
													<ul>
														{foreach from=$group.attributes key=id_attribute item=group_attribute}
															<li>
																<input type="radio" id="{$id_attribute}" class="attribute_radio" name="{$groupName|escape:'html':'UTF-8'}" value="{$id_attribute}" {if ($group.default == $id_attribute)} checked="checked"{/if} />
																<label class="label_radio {if ($group.default == $id_attribute)} checked{/if}" for="{$id_attribute}">{$group_attribute|escape:'html':'UTF-8'}</label>
															</li>
														{/foreach}
													</ul>
												{/if}
						...
						{if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
							...
							<div class="price">
								<p class="our_price_display" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
									...
									{/if}
                                    <span id="old_price"{if (!$product->specificPrice || !$product->specificPrice.reduction) && $group_reduction == 1} class="hidden"{/if}>
    									{if $priceDisplay >= 0 && $priceDisplay <= 2}
						...
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="add_to_cart" class="buttons_bottom_block no-print btnbgcolor btnbgcolorhover">
								<button type="submit" name="Submit" class="exclusive btnbgcolor btnbgcolorhover">
									<span class="btnbgcolor btnbgcolorhover">{l s='Add to cart'}</span>
								</button>
							</p>
						</div>
