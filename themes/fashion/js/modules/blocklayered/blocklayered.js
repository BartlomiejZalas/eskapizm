...
function reloadContent(params_plus)
{
	...
	if(($("#left_column").length >0 || $("#right_column").length >0) && !($("#left_column").length >0 && $("#right_column").length >0))
        className = 'ajax_block_product col-xs-12 col-sm-6 col-md-4';
    else{
        if($("#left_column").length >0 && $("#right_column").length >0)
            className = 'ajax_block_product col-xs-12 col-sm-12 col-md-6';
        else
            className = 'ajax_block_product col-xs-12 col-sm-4 col-md-3';
    } 
	ajaxQuery = $.ajax(
	{
		type: 'GET',
		...
		success: function(result)
		{
			...
			if (result.productList){
			     ...
			     $('#center_column .product_list li').removeAttr('class');
                 $('#center_column .product_list li').addClass(className);
			}else
				$('.product_list').html('');
