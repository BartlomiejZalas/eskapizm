...
$(document).ready(function(){
	...
	if ('ontouchstart' in document.documentElement)
	{
		...
		$("#header .shopping_cart a:first").hover(
			function(){
			 //alert("aaaa"+ajaxCart.nb_total_products+'VS'+cart_qty);
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('dt[data-id="cart_block_product_' + domIdProduct + '"]').length == 0)
				{
					...
					var content =  '<dt class="unvisible mainHoverColor" data-id="cart_block_product_' + domIdProduct + '">';
					var name = $.trim($('<span />').html(this.name).text());
					name = (name.length > 12 ? name.substring(0, 10) + '...' : name);
					content += '<a class="cart-images axanBorderHoverOnly" href="' + this.link + '" title="' + name + '"><img  src="' + this.image_cart + '" alt="' + this.name +'"></a>';
